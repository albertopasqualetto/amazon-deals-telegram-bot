...
def get_deals_urls():
    ...
    deals_urls = apa.get_deals_urls(selenium_driver)  # get deals only once
...
def get_random_product_info(selenium_driver, deals_urls, used_products_ids):
    ...
    while True:  # get new product until is the selected one is valid
        ...
        if (selected_product_info is not None) and (
                selected_product_info["product_id"] not in used_products_ids):  # product valid and not already sent
            break
